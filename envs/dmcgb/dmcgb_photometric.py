...
dmcgb_photometric_modes = ['color_easy', 'color_hard', 'video_easy', 'video_hard', 'video_hard_generate', 'color_video_easy', 'color_video_hard']
...
class ColorVideoWrapper(dm_env.Environment):
    ...
    def __init__(self, env, mode, seed, video_render_size=256):
        ...
        # Mode
        self._color_in_effect= 'color' in self._mode or 'generate' in self._mode
        self._video_in_effect= 'video' in self._mode
        self._remove_ground_and_rails = (self._mode == 'video_hard') or (self._mode == 'video_hard_generate')
...
# -------------------Video Helpers--------------------------------------------
    ...
    def _get_video_paths(self):
        ...
        elif 'hard' in self._mode and 'generate' not in self._mode:
        ...
        elif 'generate' in self._mode:
            video_dir = os.path.join(dmcgb_data_dir, 'video_hard_generate')
            self._video_paths = [os.path.join(video_dir, f'video{i}.mp4') for i in range(100)]
        else:
