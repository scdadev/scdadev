...
def make_agent(obs_spec, action_spec, cfg):
    ...
    return hydra.utils.instantiate(cfg)    # config-->agent_cfg
...
class Workspace:
    def __init__(self, cfg):
        ...
        self.cfg = cfg
        self.logdir = self.cfg.logdir + f'/seed{self.cfg.seed}'
        ...
        global timeshot
        timeshot = str(time.time())[-6:]
        self.replay_storage = ReplayBufferStorage(data_specs,
                                                  self.work_dir / 'buffer'/ f"{cfg.task}_seed{cfg.seed}_{timeshot}")
        self.replay_loader = make_replay_loader(
            self.work_dir / 'buffer'/ f"{cfg.task}_seed{cfg.seed}_{timeshot}", self.cfg.replay_buffer_size,
            self.cfg.batch_size, self.cfg.replay_buffer_num_workers,
            self.cfg.save_snapshot, self.cfg.nstep, self.cfg.discount)
        self._replay_iter = None

        tb_dir = f"{str(self.work_dir)}/logs/{self.logdir}_{timeshot}/tb"
        self.cgf_info_dir = f"{str(self.work_dir)}/logs/{self.logdir}_{timeshot}"
        self.writter = SummaryWriter(tb_dir)
        # tb_dir_loss = f"{str(self.work_dir)}/logs/{self.logdir}_{timeshot}/tb_loss"
        # self.writter_loss = SummaryWriter(tb_dir_loss)
        ...
        # create logger
        self.logger = Logger(self.work_dir, self.cfg, timeshot=timeshot, logdir=self.logdir)
        # create video
        self.video_recorder = VideoRecorder(save_video=self.cfg.save_video, video_dir=f"{self.cfg.video_dir}/{cfg.agent}_{cfg.task}/seed{cfg.seed}_{timeshot}")

        # add places365 dataset directory for overlay aug
        utils.add_aug_directory(self.cfg.datasets)
        
        self.info = {'task': self.cfg.task, 'seed': self.cfg.seed, 'aug': self.cfg.strong_augs[0], 
                     'sigma': self.cfg.fda_sigma, 'en_para': self.cfg.en_para, 'sigma_para': self.cfg.sigma_para}
    ...
    def write_config_to_file(self, info_dir, filename='cfg.info'):

        config_path = info_dir + '/'+ filename
        
        with open(config_path, 'w') as configfile:
            for key in self.cfg.keys():
                if key != 'agent_cfg':
                    try:
                        configfile.write(f"{key}: {self.cfg[key]}\n")
                    except:
                        continue
                else:
                    for key_agent in self.cfg.agent_cfg.keys():
                        try:
                            configfile.write(f"--{key_agent}: {self.cfg.agent_cfg[key_agent]}\n")
                        except:
                            continue

        print(f"Configuration has been written to {config_path}")
    ...
    def eval(self):
        ...
        for each_env in self.eval_envs:
            ...
            self.writter.add_scalar(each_env._mode + '_reward', metrics['episode_reward'], self.global_frame//self.cfg.action_repeat)
            # logging
    ...
    def train(self):
        # predicates
        self.write_config_to_file(self.cgf_info_dir)
        ...
        time_step = self.train_env.reset()    # time_step 环境返回
        ...
        while train_until_step(self.global_step):
            if time_step.last():
                ...
                self.replay_storage.add(time_step)
                # save snapshot
                # if self.cfg.save_snapshot and self.global_step%500000==0 and self.global_step>0:
                #     self.save_snapshot()
                if self.global_step%500000==0 and self.global_step>0:
                    self.save_snapshot()
                episode_step = 0
                episode_reward = 0
            ...
            if not seed_until_step(self.global_step):
                agent_metrics.update(self.agent.update(self.replay_iter, self.global_step, self.info))

                if 'trace_dict' in agent_metrics and self.global_step%5000==0:
                    if agent_metrics['trace_dict'] != None:
                        for key, value in agent_metrics['trace_dict'].items():
                            self.writter.add_scalar(key, value, self.global_step)
                        if 'critic_loss' in agent_metrics:
                            self.writter.add_scalar('critic_loss', agent_metrics['critic_loss'], self.global_step)
                        if 'spda_loss' in agent_metrics:
                            self.writter.add_scalar('spda_loss', agent_metrics['spda_loss'], self.global_step)                    
    ...
    def save_snapshot(self): 
        snapshot = self.work_dir /'logs'/ f'{self.logdir}_{timeshot}' /'snapshot.pt'
