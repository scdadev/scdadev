...
class Critic(nn.Module):
    ...
    def forward(self, obs, action, step=10, info=None):
        ...
        with torch.no_grad():
            ...
            uniform = torch.empty_like(h2).uniform_(-1, 1)
            # ===================================umap_figure==================================
            if (step >= 100000 and step%100000==0) or step==490000:
                ...
                length = h1.shape[0]
                scatters = umap_model.fit_transform(torch.cat([h2, uniform, h1], dim=0).cpu())
                df = pd.DataFrame(scatters, columns=['x','y'])
                df['label'] = None
                df['label'][:length] = 'obs_aug'
                df['label'][length:length*2] = 'uniform'
                df['label'][length*2:] = 'origin'
                ...
                if info is not None:
                    plt.savefig(f"./figures_feature_show_svea/{info['task']}_seed{info['seed']}_para{str(info['en_para']).replace('.','')}_{info['aug']}_{step}.png", dpi=300)
                plt.close()
        return q1, q2
...
class AGENT:
    ...
    def update_critic(self, obs, action, reward, discount, next_obs, aug_obs, step, info=None):
        metrics = dict()
        with torch.no_grad():
    ...
    def update(self, replay_iter, step, info= None):
        ...
        # update critic
        metrics.update(self.update_critic(obs, action, reward, discount, next_obs, aug_obs, step=step, info=info))
