...
class AGENT:
    ...
    def update_critic(self, obs, action, reward, discount, next_obs, aug_obs):
        ...
        # =====================================spda========================================
        if self.add_spda:
            aug_obs_cov_matrix = torch.cov(self.critic.trunk(aug_obs).T)
            spda_loss = -torch.trace(aug_obs_cov_matrix)*self.spda_para
            if self.add_clip:
                with torch.no_grad():
                    lim = critic_loss*self.clip_para
                    ratio = 1
                    if abs(spda_loss) > lim:
                        ratio = lim / abs(spda_loss)
                spda_loss = spda_loss * ratio
            total_loss = critic_loss + spda_loss
        # =================================================================================
        ...
        metrics['spda_loss'] = spda_loss.item()
    ...
    def update(self, replay_iter, step, info= None):
        ...
        # augment
        obs = self.aug(obs.float())    # random_shift
        aug_obs = self.apply_strong_aug(obs.clone())    # strong_augs
        next_obs = self.aug(next_obs.float())

        # ===================== show aug_obs =====================
        # plt.imsave('aug_obs_70.png',np.moveaxis(np.array(aug_obs[70][:3].cpu())/255.01,0,-1))
